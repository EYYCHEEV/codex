...
- `turn/responseAttempt/reset` — `{ threadId, turnId }` when incomplete model text or reasoning from a failed transport attempt must be discarded before replacement output. Clients must remove the active in-progress item and its accumulated deltas for that turn.
...
- `account/read` — fetch current account info; optionally refresh managed tokens.
- `account/login/start` — begin login (`apiKey`, `chatgpt`, `chatgptDeviceCode`, or the unstable internal `chatgptAuthTokens` form).
- `account/login/completed` (notify) — emitted when a login attempt finishes (success or error); successful API-key and externally supplied-token logins use `loginId: null`.
...
- `account/rateLimits/read` — fetch the backward-compatible rate-limit snapshot, any per-limit snapshots, and earned rate-limit reset credits.
- `account/rateLimits/updated` (notify) — sparse rolling rate-limit update; merge the available values into the latest `account/rateLimits/read` response or refetch that snapshot.
...
{ "id": 1, "result": { "account": { "type": "chatgpt", "email": "person@example.com", "planType": "pro" }, "requiresOpenaiAuth": true } }
