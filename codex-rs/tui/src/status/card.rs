...
use super::managed_accounts::managed_account_lines;
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let managed_accounts = match self.account.as_ref() {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ManagedChatGpt(accounts) => match accounts.len() {
                1 => "1 managed ChatGPT account".to_string(),
                count => format!("{count} managed ChatGPT accounts"),
            },
        ...
        lines.push(Line::from(Vec::<Span<'static>>::new()));
        // Hide thread token usage only for ChatGPT subscribers. Managed account
        // quota/usage below is account-scoped and intentionally separate from `/usage`.
        if !matches!(
            self.account,
            Some(StatusAccountDisplay::ChatGpt { .. } | StatusAccountDisplay::ManagedChatGpt(_))
        ) {
        ...
        if let Some(accounts) = managed_accounts {
            lines.push(Line::default());
            lines.extend(managed_account_lines(accounts, available_inner_width));
        } else {
