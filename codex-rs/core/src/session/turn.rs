...
async fn run_sampling_request(
    ...
    loop {
        ...
        let resource_client =
            codex_mcp::McpResourceClient::new(attempt_step_context.mcp.manager_arc());
...
pub(crate) async fn built_tools(
    ...
    let auth = tool_suggest_is_enabled
        .then(|| step_context.effective_auth.clone())
        .flatten();
    let directory_cache_key = step_context
        .mcp
        .connector_directory_cache_key(turn_context.config.chatgpt_base_url.clone(), auth.as_ref());
...
fn can_fallback_rewindable_websocket_attempt(
    ...
    replay_state.is_rewindable()
        && matches!(err, CodexErr::WebsocketClosed(_))
        && err.is_retryable()
...
mod replay_safety_tests {
    ...
    fn retryable_failure_after(event: ResponseEvent) -> AttemptOutcome<()> {
        AttemptOutcome::new(
            Err(CodexErr::Stream("retryable test failure".to_string(), None)),
