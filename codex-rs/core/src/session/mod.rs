...
impl Codex {
    ...
    pub(crate) async fn mcp_startup_snapshot(&self) -> Option<McpStartupSnapshot> {
        self.session.mcp_startup_snapshot().await
    }
...
impl Session {
    ...
    pub(crate) async fn capture_step_context(
        ...
        self.capture_step_context_inner(turn_context, None).await
    }
    ...
    pub(crate) async fn capture_step_context_for_setup(
        ...
        self.capture_step_context_inner(turn_context, Some(setup))
            .await
    }
    ...
    async fn capture_step_context_inner(
        self: &Arc<Self>,
        turn_context: Arc<TurnContext>,
        setup: Option<&CurrentClientSetup>,
    ) -> Arc<StepContext> {
        let resolved_setup;
        ...
        let mcp = self
            ...
            .await;
        let effective_auth = mcp.effective_auth().cloned();
    ...
    pub(crate) async fn build_initial_context_with_world_state(
        ...
        let mcp = self.services.try_latest_mcp_runtime();
